...
# and the initial state guess for the first time step
@model function double_pendulum(T, s_start)
  ...
  # We assume a relatively high state-transition precision
  # `zeros(4)` refers to a zeroed vector of length 4
  # `I(4)` refers to a diagonal matrix of size 4x4
  μσ = zeros(4)
  Σσ = 1000 * I(4)
  # The observational function `g` is the dot product with the vector `c`
